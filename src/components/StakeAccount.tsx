      ...
      <Card variant="outlined">
        <CardContent>
          ...
          <Typography variant="h6" component="h2">
            {`Balance: ${stakeAccountMeta.lamports / LAMPORTS_PER_PANO} PANO`} 
          </Typography>
          ...
          <Button onClick={() => setRewardsOpen(!rewardsOpen)}>
            Rewards {totalRewards / LAMPORTS_PER_PANO} PANO, {(APY && formatPct.format(APY)) || '-'} APY
          ...
          <Collapse in={rewardsOpen} timeout="auto" unmountOnExit>
            <List component="div" disablePadding>
              ...
              <ListItem style={{paddingLeft: 4}} key={inflationReward.epoch}>
                <ListItemText primary={`Epoch: ${inflationReward.epoch}, reward: ${inflationReward.amount / LAMPORTS_PER_PANO} PANO`} />
              </ListItem>
              ))}
            </List>
          </Collapse>
...
function WithdrawDialog({wallet, userPublicKey, stakePubkey, stakeAccountLamports, handleClose}: WithdrawDialogProps) {
  ...
  const max = useMemo(() => {
    return stakeAccountLamports / LAMPORTS_PER_PANO;
            ...
            await monitorTransaction(
              sendTransaction(
                ...
                StakeProgram.withdraw({
                  ...
                  lamports: Number(amount) * LAMPORTS_PER_PANO,
                }).instructions,
                []
              ),
              {
